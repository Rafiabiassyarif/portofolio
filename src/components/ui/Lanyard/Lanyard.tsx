...
export default function Lanyard({
  ...
  return (
    <div className={`relative z-0 w-full h-full flex justify-center items-center transition-opacity duration-1000 ${isReady ? 'opacity-100' : 'opacity-0'}`}>
      <Canvas
        camera={{ position, fov }}
      ...
      >
        ...
        <Physics gravity={gravity} timeStep={isMobile ? 1 / 20 : 1 / 40}>
          <Band
            isMobile={isMobile}
...
function Band({
  ...
  useEffect(() => {
    // Report readiness after a short physics initialization delay
    if (onReady) {
